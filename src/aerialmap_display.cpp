...
void AerialMapDisplay::loadImagery()
{
  ...
  if (!lastTileId || !(tileId == *lastTileId))
  {
    lastTileId = tileId;
...
{
  if (!isEnabled() || !dirty_ || !lastTileId)
  {
    return;
  }
  dirty_ = false;
  ...
  if (objects_.empty())
  {
    ...
    tileCache_.request({ *lastTileId, blocks_ });
  }
  ...
  for (int xx = area.leftTop.x; xx <= area.rightBottom.x; ++xx)
  {
    for (int yy = area.leftTop.y; yy <= area.rightBottom.y; ++yy)
    {
      ...
      double const x = (xx - lastTileId->coord.x) * tile_w_h_m;
      double const y = -(yy - lastTileId->coord.y) * tile_w_h_m;
